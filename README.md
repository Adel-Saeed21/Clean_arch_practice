...
- `remote_datasource.dart` – Communicates with APIs / server.  
- `local_datasource.dart` – Manages local storage (Hive).  
- `product_model.dart` – Data model for API responses.  
- `products_repo_impl.dart` – Repository implementation connecting domain with data sources.

---

### **2️ Domain Layer**
Defines the core logic contracts:  
- `products_repo.dart` – Abstract repository interface implemented in the Data Layer.  
*(UseCases can be added later if the project grows.)*

---

### **3️ Presentation Layer**
UI and State Management:  
- `product_cubit.dart`, `product_state.dart` – Handle product states and business logic for UI.  
...
- Dart  
- Cubit / Bloc  
- Dio or Retrofit (for networking)  
